...
func homeHandler(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := GetcomentDB().Query("SELECT info, user FROM coment")
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&comment.Info, &comment.User); err != nil {
...
func whatis(w http.ResponseWriter, r *http.Request) {

	tmpl, err := template.ParseFiles("web/whatis.html")
	if err != nil {
		handleError(w, err, "Ошибка загрузки шаблона", http.StatusInternalServerError)
		return
	}
...
func addCommentHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		if err := validateFields(info); err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}

		_, err := GetcomentDB().Exec("INSERT INTO coment (info, user) VALUES (?, ?)", info, username)
		if err != nil {
			handleError(w, err, "Ошибка базы данных", http.StatusInternalServerError)
			return
		}
	...
	http.ServeFile(w, r, "web/comment.html")
}
